...
-   **v0.1.3** (2025-09-29)
    -   **【重要修复】** 修复每日人品逻辑重大缺陷：
        -   **问题分析**：原 `get_today_key()` 方法返回精确到秒的时间戳，导致同一天内多次调用会产生不同的key，违反了"每日只能测试一次"的核心逻辑
        -   **修复方案**：
            - 修复 `get_today_key()` 方法，现在只返回日期格式（YYYY-MM-DD）
            - 新增 `get_current_timestamp()` 方法，用于记录详细的测试时间
            - 更新存储逻辑，历史记录使用详细时间戳作为key，但每日限制判断只基于日期
            - 修复删除和清除逻辑，正确处理基于时间戳的历史记录匹配
        -   **测试验证**：添加完整的测试套件验证修复效果，确保：
            - 同一天内多次调用 `get_today_key()` 返回相同结果
            - 同一用户同一天只能生成一次人品值
            - 历史记录正确保存详细时间信息用于显示
            - hash算法在同一天为同一用户生成一致的结果
        -   **影响范围**：此修复确保了插件核心功能的正确性，历史数据保持兼容
