...
class DailyFortunePlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        self.config.setdefault("rank_display_count", 10)  # 新增：排行榜显示数量
    ...
    async def fortune_rank(self, event: AstrMessageEvent):
        ...
        async with _fortune_lock:
            try:
                ...
                # 获取配置的显示数量
                display_count = self.config.get("rank_display_count", 10)
                ...
                # 如果display_count为-1，显示所有；否则显示指定数量
                if display_count == -1:
                    display_list = sorted_fortunes
                else:
                    display_list = sorted_fortunes[:display_count]

                for idx, (user_id, data) in enumerate(display_list):
